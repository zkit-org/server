...
import org.zkit.support.server.message.configuration.AliDMConfiguration;
...
public class AliDMService {
    ...
    public void singleSendMail(SendMailRequest sendRequest){
        ...
        try {
            ...
            request.setToAddress(sendRequest.getTo());
            //可以给多个收件人发送邮件，收件人之间用逗号分开，批量发信建议使用BatchSendMailRequest方式
            //request.setToAddress("邮箱1,邮箱2");
            Map<String, Object> data = sendRequest.getData();
            data.put("title", template.getTitle());
            request.setSubject(template.getTitle());
            //如果采用byte[].toString的方式的话请确保最终转换成utf-8的格式再放入htmlbody和textbody，若编码不一致则会被当成垃圾邮件。
            //注意：文本邮件的大小限制为3M，过大的文本会导致连接超时或413错误
            String html = freemarkerService.render(sendRequest.getTemplate(), template.getContent(), data);
            log.info(html);
