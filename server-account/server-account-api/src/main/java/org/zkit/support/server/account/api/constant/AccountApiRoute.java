package org.zkit.support.server.account.api.constant;

public class AccountApiRoute {

    public static final String AUTH_FIND_BY_USERNAME = "/internal/auth/account/find/by/username";
    public static final String AUTH_ACCOUNT_ADD = "/internal/auth/account/add";
    public static final String AUTH_ACCOUNT_ADD_OR_GET = "/internal/auth/account/add/or/get";
    public static final String AUTH_CREATE_TOKEN = "/internal/auth/create/token";
    public static final String AUTH_ACCOUNT_OTP_SECRET = "/internal/auth/account/otp/secret";
    public static final String AUTH_ACCOUNT_REGISTER = "/internal/auth/account/register";
    public static final String AUTH_ACCOUNT_LOGIN = "/internal/auth/account/login";
    public static final String AUTH_ACCOUNT_LOGOUT = "/internal/auth/account/logout";
    public static final String AUTH_ACCOUNT_RESET_PASSWORD = "/internal/auth/account/reset/password";
    public static final String AUTH_ACCOUNT_CHANGE_PASSWORD = "/internal/auth/account/change/password";
    public static final String AUTH_ACCOUNT_OTP_STATUS = "/internal/auth/account/otp/status";
    public static final String AUTH_ACCOUNT_OTP_BIND = "/internal/auth/account/otp/bind";
    public static final String AUTH_ACCOUNT_OTP_DISABLE = "/internal/auth/account/otp/disable";

}
